...
func _assign_new_active_pcam(pcam: Node) -> void:
	...
	if _is_2D:
		...
		## - Needs 3D variant once Godot supports physics_interpolation for 3D scenes.
		...
		Engine.get_version_info().minor >= 3:
			...
			if _physics_based:
				...
				## - Temporary solution to support Godot 4.2
...
func _pcam_follow(delta: float) -> void:
	...
	#- Should be able to find a more efficient way using signals
