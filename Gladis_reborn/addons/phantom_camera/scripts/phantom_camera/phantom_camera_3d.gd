...
func _process_logic(delta: float) -> void:
	...
	if _should_follow:
		_follow(delta)
	if _should_look_at:
		_look_at() #  - Delta needs to be applied, pending Godot's 3D Physics Interpolation to be implemented
...
func set_priority(value: int) -> void:
	priority = abs(value) # - Make any minus values be 0
	if _has_valid_pcam_owner():
		get_pcam_host_owner().pcam_priority_updated(self)
...
func _check_physics_body(target: Node3D) -> void:
	if target is PhysicsBody3D:
		...
		return
		## - Enable once Godot supports 3D Physics Interpolation
